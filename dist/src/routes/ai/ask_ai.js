...
const usageLimiter_1 = require("../../utils/usageLimiter");
...
router.post('/', async (req, res, next) => {
    try {
        ...
        const { allowed, message } = await (0, usageLimiter_1.checkUsageLimit)(userId, 'ask_count');
        if (!allowed) {
            res.status(403).json({ error: message });
            return;
        }
        // Step 1: Call LLaMA to extract filters
        const llamaResponse = await (0, llamaClient_1.askLLAMA)(prompt);
        let filters;
        try {
        ...
        const { location, budget, purpose, preferences, zip, squareFootage, yearBuilt, min_area, min_year_built, max_area, max_year_built, } = filters;
        let query = supabaseClient_1.supabaseAdmin
            .from('enriched_properties')
            .select('*')
            .limit(100);
        if (budget)
            query = query.lte('askingPrice', budget);
        if (min_area)
            query = query.gte('squareFootage', min_area);
        if (max_area)
            query = query.lte('squareFootage', max_area);
        if (!min_area && squareFootage)
            query = query.lte('squareFootage', squareFootage);
        if (min_year_built)
            query = query.gte('yearBuilt', min_year_built);
        if (max_year_built)
            query = query.lte('yearBuilt', max_year_built);
        if (!min_year_built && yearBuilt)
            query = query.gte('yearBuilt', yearBuilt);
        if (location)
            query = query.ilike('address', `%${location}%`);
        if (zip)
            query = query.eq('zip', zip);
        if (preferences?.includes('walkability'))
            query = query.gte('walkScore', 50);
        if (preferences?.includes('transit'))
            query = query.gte('transitScore', 40);
        if (preferences?.includes('bike'))
            query = query.gte('bikeScore', 40);
        ...
        if (scored.length > 0) {
            try {
                const { data: userData, error: fetchErr } = await supabaseClient_1.supabaseAdmin
                    .from('users')
                    .select('ask_count')
                    .eq('id', userId)
                    .single();
                if (fetchErr || !userData) {
                    ...
                    const { error: updateErr } = await supabaseClient_1.supabaseAdmin
                        .from('users')
