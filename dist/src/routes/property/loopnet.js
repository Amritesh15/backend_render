...
router.get('/live', async (req, res, next) => {
    try {
        ...
        const searchResponse = await axios_1.default.post('https://loopnet-api.p.rapidapi.com/loopnet/v2/lease/searchByAddress', payload, {
            headers: {
        ...
        for (const batch of batches) {
            ...
            const bulkDetailsResponse = await axios_1.default.post('https://loopnet-api.p.rapidapi.com/loopnet/property/bulkDetails', { listingIds: batch }, {
                headers: {
                    'X-RapidAPI-Key': process.env.RAPIDAPI_KEY,
                    'X-RapidAPI-Host': 'loopnet-api.p.rapidapi.com',
                    'Content-Type': 'application/json',
        ...
        const simplified = allDetails.map((item) => {
...
router.post('/populate', async (req, res, next) => {
    try {
        ...
        while (page <= maxPages) {
            ...
            const liveResponse = await axios_1.default.get(`http://localhost:${process.env.PORT || 5050}/api/property/live?${params.toString()}`);
        ...
        const enrichedListings = allListings.map((item) => ({
            id: item.id,
